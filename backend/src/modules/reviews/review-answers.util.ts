...
export function validateReviewAnswers(
  ...
  for (const qid of answersMap.keys()) {
    if (!validIds.has(qid)) throw new BadRequestException(`Invalid questionId: ${qid}`)
  }

  if (!skipRequired) {
    const missing = questions.filter((q) => q.required && !answersMap.get(q.id)?.trim())
    if (missing.length > 0) {
      throw new BadRequestException(`以下必填題尚未回答：${missing.map((q) => q.questionText).join('、')}`)
    }
  }
  ...
  for (const q of questions) {
    ...
    validateAnswerFormat(q, raw)
  }
}

function validateAnswerFormat(q: TemplateQuestion, raw: string): void {
  ...
  if (q.questionType === QuestionType.MultipleChoice && !q.options.includes(raw)) {
