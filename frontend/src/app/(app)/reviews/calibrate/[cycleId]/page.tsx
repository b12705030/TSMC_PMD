...
export default function CalibratePage({ params }: { params: Promise<{ cycleId: string }> }) {
  ...
  const { reviews: serverReviews, isLoading, refetch } = useCycleReviews(cycleId)
      ...
      <td className="px-4 py-3">
        {isPending ? (
          <input
            ...
            placeholder="—"
            className="w-16 rounded-lg border border-gray-200 px-2 py-1 text-center text-sm outline-none focus:border-indigo-300 focus:ring-1 focus:ring-indigo-200"
          />
        ) : (
